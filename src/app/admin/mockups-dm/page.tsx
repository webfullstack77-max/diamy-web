...
type Step = "browse" | "design" | "result";
...
function extractArray(data: unknown): unknown[] {
  ...
  if (data && typeof data === "object") {
    ...
    for (const key of ["mockups", "items", "results", "templates"]) {
...
function getMockupName(m: RawMockup): string {
  return (m.name ?? m.title ?? m.label ?? "Sin nombre") as string;
}

function getMockupUuid(m: RawMockup): string {
  return (m.uuid ?? m.id ?? "") as string;
}
...
export default function MockupsDMPage() {
  ...
  const fetchMockups = useCallback(async (q: string, p: number, append = false) => {
  ...
  useEffect(() => { fetchMockups("", 1); }, [fetchMockups]);

  function handleSearch(q: string) {
    setSearch(q);
    setPage(1);
    if (searchRef.current) clearTimeout(searchRef.current);
    searchRef.current = setTimeout(() => fetchMockups(q, 1), 400);
  }

  function loadMore() {
    const next = page + 1;
    setPage(next);
    fetchMockups(search, next, true);
  }
  ...
  async function handleRender() {
    ...
    try {
      const r = await fetch("/api/admin/dynamic-mockups/renders", {
        ...
        body: JSON.stringify({
          mockup_uuid: getMockupUuid(selectedMockup),
  ...
  const STEPS = ["Elegir template", "Subir diseño", "Resultado"];
  const STEP_INDEX: Record<Step, number> = { browse: 0, design: 1, result: 2 };
    ...
    <div className="max-w-4xl space-y-6">
          ...
          return (
            <div key={label} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${done ? "bg-primary text-on-primary" : active ? "bg-primary text-on-primary ring-2 ring-primary/30" : "bg-surface-container text-on-surface-muted"}`}>
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Buscar template: hoodie, mug, tumbler, t-shirt…"
          ...
          {mockupsError && (
            <div className="bg-error-container/20 border border-error/20 rounded-2xl p-4 text-sm text-error">
              {mockupsError}
              {mockupsError.includes("no configurado") && (
          ...
          {!loadingMockups && mockups.length === 0 && !mockupsError && (
            <p className="text-center py-12 text-on-surface-muted text-sm">Sin resultados para "{search}".</p>
          )}
                ...
                {mockups.map((m, idx) => {
                  ...
                  return (
                    <button key={uuid || idx} onClick={() => selectMockup(m)}
                      className="bg-surface border border-outline-variant rounded-2xl overflow-hidden text-left hover:border-primary/50 hover:shadow-sm transition group">
                      {thumb ? (
              ...
              {hasMore && (
                <div className="flex justify-center pt-2">
                  <button onClick={loadMore} disabled={loadingMockups}
                    ...
                    {loadingMockups
                      ? <><span className="material-symbol animate-spin" style={{ fontSize: 16 }}>progress_activity</span>Cargando…</>
                      : "Cargar más"}
                  </button>
                </div>
              )}
      ...
      {step === "design" && selectedMockup && (
        <div className="space-y-5">
          <button onClick={reset} className="flex items-center gap-1 text-sm text-on-surface-muted hover:text-on-surface transition">
            <span className="material-symbol" style={{ fontSize: 16 }}>arrow_back</span>
            <span className="ml-1">{getMockupName(selectedMockup)}</span>
          </button>
